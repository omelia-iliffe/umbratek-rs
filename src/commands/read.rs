use crate::{Bus, ReadError, TransferError};
        ...
        WriteBuffer: AsRef<[u8]> + AsMut<[u8]>,
{
    ...
    pub fn read<REG: Register + Readable>(&mut self, motor_id: u8) -> Result<REG::INNER, TransferError> {
        self.read_raw(motor_id, REG::address(), REG::size()).and_then(|response| {
            Ok(
                REG::try_from_bytes(response.data()).map_err(ReadError::from)?.value(),
            )
        })
    }
}
