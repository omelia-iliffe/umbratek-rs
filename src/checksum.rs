#[rustfmt::skip]
const CRC_TABLE_H: [u8; 256] = [0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0, 0x80,
...
const CRC_TABLE_L: [u8; 256] = [0x00, 0xC0, 0xC1, 0x01, 0xC3, 0x03, 0x02, 0xC2, 0xC6, 0x06, 0x07,
...
pub fn calculate_checksum(data: &[u8]) -> [u8;2] {
	...
	for &byte in data {
		let index = checksum_high ^ byte;
		checksum_high = checksum_low ^ CRC_TABLE_H[index as usize];
