...
macro_rules! register {
    (@register $reg:ident : $addr:expr, $type:ty, $size:expr) => {
        #[derive(Debug, Clone, PartialEq)]
        #[doc = concat!("[`",stringify!($reg),"`] register at address `",stringify!($addr), "`")]
        #[doc = concat!("[`",stringify!($reg),"`] is of type [`", stringify!($type), "`]")]
        pub struct $reg($type);
    ...
    (@RW_TYPE $reg:ident : $addr:expr, $type:ty, RW, $size:expr) => {
        register!(@register $reg : $addr, $type, $size);
        register!(@writable $reg : $addr, $type);
        register!(@readable $reg : $addr, $type);
    };
    (@RW_TYPE $reg:ident : $addr:expr, $type:ty, R, $size:expr) => {
        register!(@register $reg : $addr, $type, $size);
        register!(@readable $reg : $addr, $type);
    };
    (@RW_TYPE $reg:ident : $addr:expr, $type:ty, W, $size:expr) => {
        register!(@register $reg : $addr, $type, $size);
        register!(@writable $reg : $addr, $type);
    };
	($reg:ident : $addr:expr, $type:ty, $rw_type:ident) =>
    {
        register!(@RW_TYPE $reg : $addr, $type, $rw_type, std::mem::size_of::<$type>() as u8);
    };
	($reg:ident : $addr:expr, $type:ty, $rw_type:ident, $size:expr) =>
	{
		register!(@RW_TYPE $reg : $addr, $type, $rw_type, $size);
	};
...
pub mod registers {
    ...
    register!(ErrorCode: 0x2F, ErrorCodes, R, 1);
...
pub enum ErrorCodes {
    ...
    EncoderCalculateError,
    // Keep motor stationary during power on,
    EncoderError1,
    EncoderError2,
    ...
    PhaseBAlarm,
    PhaseCAlarm,
    PositionFollowingAlarm,
    // Auto cleared when torque enabled
